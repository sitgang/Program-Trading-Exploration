    ...
    for dateid in date_array:
        ...
        if len(codes)>0:
            ...
            for code in codes:
                ...
                try:
                    now_price=df[df['code']==int(code)].loc[dateid,'close']
                except KeyError:
                    now_price=in_price
            ...
            #account.cash_action[dateid]=account.cash_amount
        #print dateid,account.cash_amount,account.stock_amount
    return account      
...
class Account(object):
    ...
    def test_report(self):
        ...
        starting_point=min(df.in_datetime)
        ending_point=max(df.index)
        #回测开始结束时间
        average_position=int(np.mean(df.quantity))
        #平均持仓量
        max
